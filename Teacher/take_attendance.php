...
<script src="https://cdnjs.cloudflare.com/ajax/libs/jquery/3.3.1/jquery.min.js"></script> 
 ...
 while( $row=mysqli_fetch_assoc($s)){
  ...
  if(isset($_POST['submit'])){
    ...
    if($flag==0){
    $folderPath = "Teacher/staff/"; 
    $image_parts = explode(";base64,", $img); 
    $image_type_aux = explode("Teacher/staff/image/", $image_parts[0]); 
...
$sql5="SELECT Subject_id from subject where Subject_name='$subname'";
  $sq=mysqli_query($con,$sql5);
 while( $row=mysqli_fetch_assoc($sq)){
  $subid=$row['Subject_id'];

 }
$sql="INSERT INTO idtable(subid) values('$subid')";
